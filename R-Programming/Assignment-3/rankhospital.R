...
## function: 
##   rankhospital - Ranking hospitals by outcome in a state
## parameters:
...
rankhospital <- function(state, outcome, num = "best") {
    ...
    outcom_col <- grep(arg2, outcome_names);
    ...
    rs <- sub_oocm_data[ ,c(outcom_col, 2)]
    
    ## convert outcome to numeric
    suppressWarnings( rs[,1]<-as.numeric(rs[,1]) )
    rs<-subset(rs,!is.na(rs[,1]))
    
    ## order the result by outcome, hospital name
    rc <- rs[order(rs[,1],rs[,2]),2]
    
    ## return the result
    if (num == "best") head(rc,1)
    else if (num == "worst") tail(rc,1)
    else rc[num]
}